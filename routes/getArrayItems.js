let arrayItems = require('express')();
...
arrayItems.get('/getItems', function (req, res) {
    if (req.query.id) {
        let data = jsonData.find((ele) => ele.index == req.query.id);
        if (data != undefined) {
            sendResult(data)
        }
        else {
    ...
    function sendResult(jsonVal) {
        return res.send(jsonVal)
    }
...
arrayItems.get('/getItems/:id', function (req, res) {
    if (req.params.id) {
        let data = jsonData.find((ele) => ele.index == req.params.id);
        if (data != undefined) {
            sendResult(data)
        }
        else {
